...
export default function ChatPage() {
  ...
  useOnlineStatus({ userId: user?.id });
  ...
  useEffect(() => {
    const handleConversationRead = (event: Event) => {
      const detail = (event as CustomEvent<{ conversationId: string; messageIds: string[] }>).detail;
      if (!detail) return;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto h-[calc(100vh-8rem)] md:h-[calc(100vh-6rem)] px-0 sm:px-4">
      <div className="flex flex-col lg:flex-row gap-4 h-full">
        ...
        <div className={`${activeId ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-80 h-full gap-4`}>
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-none sm:rounded-[32px] border-x-0 sm:border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden flex flex-col transition-colors">
            <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-xl sm:text-2xl font-black font-outfit text-slate-900 dark:text-white tracking-tight">Messages</h1>
                </div>
                <button 
        ...
        {/* Main Content Area */}
        <div className={`${!activeId ? 'hidden lg:flex' : 'flex'} flex-1 min-w-0 h-full`}>
          <div className="flex-1 flex gap-6 h-full">
            ...
            <div className="flex-1 bg-white dark:bg-slate-900 rounded-none sm:rounded-[32px] border-x-0 sm:border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden flex flex-col transition-colors">
              {activeId ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-slate-50/30 dark:bg-slate-950/10">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-xl p-6 flex flex-col gap-6 transition-colors">
                  ...
                  <div className="space-y-4">
                    ...
                    {suggestedUsers.length > 0 ? (
                      ...
                      <p className="text-xs text-slate-400 italic py-4 text-center">Pas de suggestions.</p>
                    )}
                  </div>
